...
class BooksSerializer(ModelSerializer):
    ...
    class Meta:
        model = Book
        fields = (
            'id', 'name', 'price', 'author_name', 'annotated_likes',
            'rating', 'owner_name', 'readers')

    # Первый способ
    # добавть в field 'likes_count'
    # def get_likes_count(self, instance):
    #     return UserBookRelation.objects.filter(book=instance,
    #                                            like=True).count()
    # будет куча sql запросов(сколько книг столько и запросов)

    # Второй способ через annotate
